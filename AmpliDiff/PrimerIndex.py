...
class PrimerIndex:
    ...
    def update_conflict_matrix(self, primers, seq_id=None):
        '''
    ...
    def check_conflict(self, primer_pair, seq_id=None):
        self.update_conflict_matrix(primer_pair, seq_id)
    ...
    def primer_similarity(self):
        ...
        '''
        print('Computing Forward Primer Similarity.')
        ...
        print('Computing Reverse Primer Similarity.')
        for primer in self.index2primer['reverse']:
            for similar_primer in self.index2primer['reverse']:
                if primer.__eq__(similar_primer):
                    continue
                if self.distance == 'hd':
                    similarity = hamming_distance(primer, similar_primer)
                ...
                if similarity <= self.mismatches:
                    # Reverse primers
                    self.similar_primers['reverse'][primer.sequence].add(similar_primer.sequence)
                    self.similar_primers['reverse'][similar_primer.sequence].add(primer.sequence)
