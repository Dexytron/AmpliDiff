...
def init_parser():
    ...
    parser.add_argument('-o', '--output', type=str, default='.',
                        help='Path to the folder where output will stored, default is current folder')
    parser.add_argument('--primer_thresholds', type=str, default='./primer_thresholds.csv',
                        help='Path to the primer thresholds file, default is ./primer_thresholds.csv')
    # Amplicon parameters
    parser.add_argument('-aw', '--amplicon_width', type=int, default=200, help='Amplicon size, default is 200')
    parser.add_argument('-mm', '--max_mismatches', type=int, default=0,
                        help='Number of allowed mismatches for amplicon differentiation, default is 0')
    parser.add_argument('-mt', '--max_misalign', type=int, default=20,
                        help='Number of allowed misalign characters in an amplicon, default is 20')
    # Primer parameters
    parser.add_argument('-pw', '--primer_width', type=int, default=25, help='Primer size, default is 25')
    parser.add_argument('-sw', '--search_width', type=int, default=50,
                        help='Search window for finding primers, default is 50')
    parser.add_argument('-cov', '--coverage', type=float, default=1.0,
                        help='Minimal required amplifiability, default is 100')
    parser.add_argument('-b', '--beta', type=float, default=0.05,
                        help='Trade-off parameter between primer pairs and differentiability, default is 0.05')
    parser.add_argument('--max_primer_degeneracy', type=int, default=4 ** 5,
                        help='Maximum allowed degeneracy for disambiguating primer candidates, default is 1024')
    parser.add_argument('--gc_lb', type=float, default=0.4,
                        help='Minimum required GC content in primers, default is 0.4')
    parser.add_argument('--gc_ub', type=float, default=0.6,
                        help='Maximum allowed GC content in primers, default is 0.6')
    parser.add_argument('--melting_lb', type=float, default=55.,
                        help='Minimum required primer melting temperature in degrees Celsius, default is 55')
    parser.add_argument('--melting_ub', type=float, default=75.,
                        help='Maximum allowed primer melting temperature in degrees Celsius, default is 75')
    parser.add_argument('--end_at_threshold', type=int, default=2,
                        help="Maximium allowed A/T nucleotides in final 3 nucleotides, default is 2")
    parser.add_argument('--end_gc_threshold', type=int, default=3,
                        help="Maximum allowed C/G nucleotides in final 5 nucleotides, default is 3")
    parser.add_argument('--monorun_threshold', type=int, default=3,
                        help='Maximum allowed number of single nucleotide runs, default is 3')
    parser.add_argument('--duorun_threshold', type=int, default=3,
                        help="Maximum allowed number of double nucleotide runs, default is 3")
    parser.add_argument('--mfe_threshold', type=float, default=-5.,
                        help='Minimum required MFE for determining hairpin formation risk, default is -5')
    parser.add_argument('--self_complementarity_threshold', type=int, default=10,
                        help='Maximum self and primer-primer complementarity in "worst" alignment, default is 10')
    parser.add_argument('--max_temperature_difference', type=float, default=5,
                        help='Maximum difference between minimum and maximum primer melting temperatures, default is 5')
    # Greedy algorithm parameters
    parser.add_argument('-amps', '--num_amplicons', type=int, default=10,
                        help='Number of amplicons to find, default is 10')
    # Sequence parameters
    parser.add_argument('-n', '--num_sequences', type=int, default=-1,
                        help='Number of sequences to include, default is -1 (no maximum)')
    parser.add_argument('--min_characters', type=int, default=-1,
                        help='Minimum number of characters in a sequence, default is -1 (no minimum)')
    parser.add_argument('--max_degeneracy', type=int, default=-1,
                        help='Maximum number of degenerate degeneracy of a sequence, default is -1 (no maximum)')
    parser.add_argument('--max_n', type=int, default=-1,
                        help='Maximum number of n characters in a sequence, default is -1 (no maximum)')
    # System parameters
...
def main():
    ...
    with open(args.output + '/runtimes_' + str(args.seed) + '.txt', 'a') as f:
        f.write('Time spent processing sequences and determining feasible amplicons: ' + str(
            time.time() - st) + ', number of feasible amplicons: ' + str(len(feasible_amplicons)) + '\n')
    ...
    try:
        with open(args.primer_thresholds, 'r') as f:
            for threshold in f:
                ...
                elif threshold[0] in ['end_at_threshold', 'end_gc_threshold', 'monorun_threshold', 'duorun_threshold',
    ...
    amplicons, differences_per_amplicon = generate_amplicons(sequences, args.amplicon_width, comparison_matrix,
                                                             ...
                                                             feasible_amplicons=feasible_amplicons,
                                                             relevant_nucleotides=relevant_nucleotides)
    with open(args.output + '/runtimes_' + str(args.seed) + '.txt', 'a') as f:
    ...
    logs, result_amplicons, result_primers = greedy_amplicon_selection(sequences, amplicons, differences_per_amplicon,
                                                                       args.primer_width, args.search_width,
                                                                       primer_index, comparison_matrix,
                                                                       args.num_amplicons, args.coverage,
                                                                       args.max_temperature_difference, logging=True,
                                                                       beta=args.beta,
                                                                       output_file=args.output + '/primers_' + str(
                                                                           args.seed) + '.fasta')
    with open(args.output + '/runtimes_' + str(args.seed) + '.txt', 'a') as f:
